...
3. `push segment i`的实现：`addr = segmentPointer+i, *SP = *addr, SP++`
   ...
   `push/pop temp i`的实现(类似segment，pointer换成5)：
...
class VMTranslator{
    ...
    // 判断命令行是否正确，输出用法并系统退出exit(1)
    // 检查提供的命令行参数，以确定它是有效的文件或目录(1.不存在，2.文件，3.目录)
    // if 1.错误处理
    // else if 2.确定是.vm文件，获取Parser对象，并解析指令参数(路径+文件名)初始化文件名; 初始化Parser对象的文件名，将其存储在数组链表
    // else if 3.根据File对象获取映射路径下所有的文件存储在数组，遍历所有文件，找出.vm文件并进行"2."的解析(Parser)，(其中Paser对象文件名设置依据各个.vm文件File对象，非arg[0])
    // 			if 若Parser对象的数组链表为空则输出错误并退出，初始化输出文件的路径+文件名
    // else 4.错误处理
    // 实例化codeWriter对象(根据codeWriter的属性printerWriter)
    // 遍历Paser对象数组链表: 
    //		初始化codeWriter对象的属性:当前解析的文件名
    //		常规逐行扫描，跳过空白，无内容行直接下一循环
    //		if 若是"push"或"pop"，writePushAndPop(Paser对象的指令类型，指令第一个参数，第二个参数)
    //		else if 若是算术指令，writeArithmetic(Paser对象中完整的指令)
    //    关闭Paser对象的资源
    // 关闭codeWriter的资源
}
```
